...
def compute_codelen(params, sigma, counts, norm, func):
    ...
    # First try: default (automatic) step
    try:
    ...
    # If default failed, sweep relative step sizes (as ESR does)
...
# Known good parameters from sim 50 (hmf_50_final_functions.txt)
LITERATURE_FUNCTIONS = {
    ...
    'War.': {
        ...
        'p0': np.array([3.954, -0.851, -0.080, 0.811]),  # from sim 50
    ...
    'Tin.': {
        ...
        'p0': np.array([0.00234, 0.794, 1224, 0.926]),  # from sim 50
...
def main():
    ...
    # Validate against known ESR fit for sim 50
    print("\nValidating data format against known ESR fit (sim 50)...")
    sigma50, counts50, norm50 = load_hmf_data(50)
    ...
    # Also validate Warren on sim 50
    ...
    # Fit
    print(f"\nFitting literature functions to {len(sim_numbers)} sims...")
    results = {name: {} for name in LITERATURE_FUNCTIONS}

    for sim_idx, sim in enumerate(sim_numbers):
        try:
            sigma, counts, norm = load_hmf_data(sim)
        except Exception as e:
        ...
        for name, info in LITERATURE_FUNCTIONS.items():
            ...
            if np.isfinite(nll) and np.isfinite(codelen):
                ...
                # Update p0 for next sim (warm start from last good fit)
                if info['nparam'] > 0:
                    info['p0'] = params.copy()
    ...
    # Combined DL
    print(f"\n{'='*80}")
    print(f"Combined results across simulations")
    print(f"{'='*80}")
    ...
    # Save results
    outfile = 'literature_fits_all_sims.txt'
    ...
    print(f"\nPer-sim results saved to {outfile}")

    outfile2 = 'literature_combined_DL.txt'
    ...
    print(f"Combined results saved to {outfile2}")
