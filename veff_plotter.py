...
Creates figures showing V_eff for the four SDSS LF/SMF datasets (Sérsic and
...
ax_lf.plot(np.log10(LF_Ser[:, 0]), LF_Ser[:, 3], 'o-', color='C0', ms=6, lw=2.0, label='Sérsic')
lf_cmod_logL = -0.4 * (LF_cmod[:, 0] - 4.67)
ax_lf.plot(lf_cmod_logL, LF_cmod[:, 3], 's-', color='C1', ms=6, lw=2.0, label='cmodel')
ax_lf.set_xlabel(r'$\log(L / L_\odot)$', fontsize=19)
...
ax_smf.plot(np.log10(SMF_Ser[:, 0]), SMF_Ser[:, 3], 'o-', color='C0', ms=6, lw=2.0, label='Sérsic')
ax_smf.plot(np.log10(SMF_cmod[:, 0]), SMF_cmod[:, 3], 's-', color='C1', ms=6, lw=2.0, label='cmodel')
ax_smf.set_xlabel(r'$\log(M_\star / M_\odot)$', fontsize=19)
...
ax1.plot(np.log10(LF_Ser[:, 0]), LF_Ser[:, 3], 'o-', color='C0', ms=6, lw=2.0, label='Sérsic')
ax1.plot(lf_cmod_logL, LF_cmod[:, 3], 's-', color='C1', ms=6, lw=2.0, label='cmodel')
ax1.set_xlabel(r'$\log(L / L_\odot)$', fontsize=19)
ax1.set_ylabel(r'$V_{\rm eff}$ [Mpc$^3$]', fontsize=19)
ax1.set_yscale('log')
ax1.legend(fontsize=15)
ax1.tick_params(labelsize=16)

ax2.plot(np.log10(SMF_Ser[:, 0]), SMF_Ser[:, 3], 'o-', color='C0', ms=6, lw=2.0, label='Sérsic')
ax2.plot(np.log10(SMF_cmod[:, 0]), SMF_cmod[:, 3], 's-', color='C1', ms=6, lw=2.0, label='cmodel')
ax2.set_xlabel(r'$\log(M_\star / M_\odot)$', fontsize=19)
ax2.set_yscale('log')
