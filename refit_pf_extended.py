...
RANK_FILE  = os.environ.get('REFIT_RANK_FILE', 'pf_candidates.txt')
...
# ── Data (EXTENDED: full new.dat, all 18 bins, NO [2:] trim) ──
...
def load_top_functions():
    ...
    with open(RANK_FILE) as f:
        for line in f:
            ...
            # complexity (wrong aifeyn too) and so dropped genuinely good functions
...
def load_paper_donors(top_funcs):
    ...
    for sim in ALL_SIMS:
        fp = f'hmf_data/hmf_{sim}_data/final_all.txt'  # EXTENDED per-sim donors
...
def main():
    ...
    # ── Deduplicate algebraically-equivalent representations by combined likelihood ──
    # Identical sum_NLL over all 100 sims => the same function under a different ESR tree.
    # Keep the minimum-DL representation (MDL: a function's description length is its
    # shortest valid encoding). results are DL-sorted, so the first member of each NLL
    # group is the minimum-DL one. This is what rescues a function's low-DL tree when its
    # cheap fits were too fragile to clear the selection's MIN_SIMS on their own.
    # Among each equivalent-NLL group keep the MINIMUM-complexity representative
    # (tie-break by min DL), rather than blindly the first DL-sorted one. This
    # guarantees a function's simplest form survives the merge and is binned at
    # its true complexity on the Pareto front (a higher-complexity equivalent must
    # not displace the low-complexity one, which previously emptied the c6-c8 bins).
    _seen, _dedup = {}, []
    for r in results:
        if r['n_sims'] >= 1 and np.isfinite(r.get('sum_NLL', np.nan)):
            key = round(r['sum_NLL'], 0)
            rank = (COMP_MAP.get(r['func'], 99), r['DL_combined'])
            if key in _seen:
                j = _seen[key]
                kept = _dedup[j]
                if rank < (COMP_MAP.get(kept['func'], 99), kept['DL_combined']):
                    _dedup[j] = r          # replace with simpler representative
                continue
            _seen[key] = len(_dedup)
        _dedup.append(r)
    n_merged = len(results) - len(_dedup)
    results = _dedup
    results.sort(key=lambda r: r['DL_combined'])   # re-sort after representative swaps
    print(f"  deduplicated {n_merged} equivalent representations "
          f"(kept min-complexity, then min-DL, of each)", flush=True)

    # ── Write combined results (with search complexity) ──
    best_DL = results[0]['DL_combined']
    sum_NLL0 = results[0]['sum_NLL']
    with open('pf_refit_combined_extended.txt', 'w') as f:
    ...
    with open('pf_refit_persim_extended.txt', 'w') as f:
